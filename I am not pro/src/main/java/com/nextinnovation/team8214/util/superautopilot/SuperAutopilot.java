...
public class SuperAutopilot {
  ...
  private Autopilot.APResult applyToleranceClamping(Autopilot.APResult result) {
    ...
    var vx = result.vx().in(MetersPerSecond);
    var vy = result.vy().in(MetersPerSecond);

    // Transform velocityMeterPerSec to entry angle coordinate frame
    var velocityVector = new Translation2d(vx, vy);
    var velocityInFrame =
        transformToEntryAngleFrame(velocityVector, lastTarget.getEntryAngle().get());

    var clampedVx = velocityInFrame.getX();
    var clampedVy = velocityInFrame.getY();

    if (atX(lastCurrentPose, lastTarget.getReference(), lastTarget.getEntryAngle().get())) {
      clampedVx = 0.0;
    }

    if (atY(lastCurrentPose, lastTarget.getReference(), lastTarget.getEntryAngle().get())) {
      clampedVy = 0.0;
    }

    var clampedVelocityInFrame = new Translation2d(clampedVx, clampedVy);
    var clampedVelocity =
        transformFromEntryAngleFrame(clampedVelocityInFrame, lastTarget.getEntryAngle().get());

    return new Autopilot.APResult(
        MetersPerSecond.of(clampedVelocity.getX()),
        MetersPerSecond.of(clampedVelocity.getY()),
        result.targetAngle());
  }
  ...
  private Translation2d transformFromEntryAngleFrame(Translation2d vector, Rotation2d entryAngle) {
    return vector.rotateBy(entryAngle);
  }
